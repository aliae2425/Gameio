...
export function LobbyPage({ onJoinMatch, onDebug }: Props) {
  ...
  const refreshMatches = async () => {
    try {
      const { matches: m } = await lobbyClient.listMatches('love-letter');
  ...
  const joinMatch = async (matchID: string) => {
    ...
    setError('');
    try {
      const match = await lobbyClient.getMatch('love-letter', matchID);
      const openSlot = (match.players as Array<{ id: number; name?: string }>).find(p => !p.name);
      if (!openSlot) { setError('La partie est complète'); setLoading(false); return; }
      const { playerCredentials } = await lobbyClient.joinMatch('love-letter', matchID, {
  ...
  const createAndJoin = async () => {
    ...
    setError('');
    try {
      const { matchID } = await lobbyClient.createMatch('love-letter', { numPlayers });
    ...
    <div className="lobby">
      ...
      <div className="debug-section">
        ...
        <div className="create-section">
          <label>
            Joueurs total :
            <select value={debugPlayers} onChange={e => setDebugPlayers(Number(e.target.value) as 2 | 3 | 4 | 5 | 6)}>
              {([2, 3, 4, 5, 6] as const).map(n => (
                <option key={n} value={n}>{n} ({n - 1} bot{n > 2 ? 's' : ''})</option>
              ))}
            </select>
          </label>
          <button className="btn-debug" onClick={() => onDebug({ debug: true, numPlayers: debugPlayers })}>
            Jouer vs Bots
          </button>
        </div>
      </div>
      ...
      <div className="lobby-form">
        ...
        <div className="create-section">
          ...
          <button onClick={createAndJoin} disabled={loading}>Créer une partie</button>
        </div>
      </div>
          ...
          openMatches.map(m => {
            ...
            return (
              <div key={m.matchID} className="match-item">
                ...
                <button onClick={() => joinMatch(m.matchID)} disabled={loading}>Rejoindre</button>
              </div>
            );
          })
        )}
